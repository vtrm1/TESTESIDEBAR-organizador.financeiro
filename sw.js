const APP_VERSION = 'of-pwa-v2025.10.30.1';
...
const MESSAGE_CHANNEL = 'of-sw-updates';
...
self.addEventListener('activate', (event) => {
  event.waitUntil((async () => {
    ...
    await enableNavigationPreloadIfSupported();
...
self.addEventListener('message', (event) => {
  ...
  if (event?.data?.type === 'GET_VERSION') {
    const message = { type: 'VERSION', version: APP_VERSION };
    if (event.source?.postMessage) {
      event.source.postMessage(message);
    } else {
      event.waitUntil(notifyClients(message));
    }
  }
});
...
async function handleNavigationRequest(event) {
  ...
  try {
    ...
    if (preloadResponse) {
      event.waitUntil(persistNavigationResponse(cache, request, preloadResponse));
      return preloadResponse;
    }
  } catch (preloadError) {
    console.warn('[SW] Falha ao usar navigation preload:', preloadError);
  }

  try {
    const networkResponse = await fetch(request);
    if (networkResponse && networkResponse.ok) {
      event.waitUntil(persistNavigationResponse(cache, request, networkResponse));
    }
...
async function notifyClients(message) {
  ...
  if (typeof BroadcastChannel !== 'undefined') {
    try {
      ...
      console.warn('[SW] BroadcastChannel indisponível:', error);
    }
  }
  ...
  await Promise.all(allClients.map(async (client) => {
    try {
      ...
      console.warn('[SW] Não foi possível enviar mensagem ao cliente:', error);
    }
  }));
}

async function enableNavigationPreloadIfSupported() {
  ...
  try {
    ...
    console.warn('[SW] Navigation preload indisponível:', error);
  }
}

async function persistNavigationResponse(cache, request, response) {
  try {
    await Promise.all([
      cache.put(request, response.clone()),
    ...
    console.warn('[SW] Não foi possível armazenar resposta de navegação:', error);
  }
}
...
function initializeFirebaseMessaging() {
  try {
    ...
    messaging.onBackgroundMessage((payload) => {
      ...
      const title = notification.title || 'ECONOMEO';
      const options = {
